...
class ClimateViz {
    ...
    async createTemperatureChart() {
        ...
        try {
            ...
            if (this.advancedFeatures) {
                filteredData = this.advancedFeatures.filterDataByTimeline(temperatureData.data);
                const locationMultiplier = this.advancedFeatures.locationData[this.advancedFeatures.filters.location]?.tempMultiplier || 1;
            ...
            this.showError('temperatureChart', 'Failed to load temperature data. Using fallback data.');
    ...
    createSeaLevelMap() {
        ...
        try {
            ...
            const vulnerableCities = [
                { name: 'Miami, FL', lat: 25.7617, lng: -80.1918, risk: 'High', projection: '0.6m by 2100' },
                { name: 'Venice, Italy', lat: 45.4408, lng: 12.3155, risk: 'Very High', projection: '1.2m by 2100' },
                { name: 'Bangkok, Thailand', lat: 13.7563, lng: 100.5018, risk: 'High', projection: '0.8m by 2100' },
                { name: 'Amsterdam, Netherlands', lat: 52.3676, lng: 4.9041, risk: 'Medium', projection: '0.4m by 2100' },
                { name: 'New Orleans, LA', lat: 29.9511, lng: -90.0715, risk: 'Very High', projection: '1.0m by 2100' },
                { name: 'Jakarta, Indonesia', lat: -6.2088, lng: 106.8456, risk: 'Very High', projection: '2.5m by 2100' },
                { name: 'Mumbai, India', lat: 19.0760, lng: 72.8777, risk: 'High', projection: '0.7m by 2100' },
                { name: 'Alexandria, Egypt', lat: 31.2001, lng: 29.9187, risk: 'High', projection: '0.9m by 2100' }
            ];

            vulnerableCities.forEach(city => {
                const color = city.risk === 'Very High' ? '#ff4444' : 
                             city.risk === 'High' ? '#ff8800' : '#ffcc00';
        ...
        try {
            ...
            g.selectAll('.bar')
                ...
                .on('mouseover', function(event, d) {
                    ...
                    tooltip.html(`<strong>${d.country}</strong><br/>${d.emissions.toFixed(2)} billion tons CO₂<br/>Year: ${d.year}`)
