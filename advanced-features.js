...
class AdvancedClimateFeatures {
    ...
    setupInteractiveControls() {
        ...
        // Globe controls
        const globeControls = document.querySelectorAll('.globe-controls .control-btn');
    ...
    applyTimelineFilter() {
        ...
        const activeSection = document.querySelector('.visualization-section.active');
        if (activeSection) {
    ...
    async createHeatmap() {
        ...
        try {
            ...
            const temperatureData = await this.app.climateAPI.getTemperatureAnomalies();
            ...
            g.append('g')
                ...
                .call(d3.axisBottom(xScale).tickValues(years.filter((d, i) => i % 5 === 0)))
                .selectAll('text')
                .style('fill', '#333')
            ...
            // Add color legend
            this.addColorLegend(svg, colorScale, width + margin.left + margin.right - 80, 100);
            ...
            this.app.showError('heatmapChart', 'Failed to create heatmap visualization');
        }
    }
    ...
    async create3DGlobe() {
        ...
        try {
            ...
            container.innerHTML = '<p style="color: #f2f2f7; text-align: center; padding: 2rem;">3D Globe visualization unavailable</p>';
        }
    }
    ...
    async createProjections() {
        ...
        const scenario = document.getElementById('scenarioSelect')?.value || 'rcp45';
        const targetYear = document.getElementById('projectionYear')?.value || '2050';

        // Temperature projections
        const tempProjections = this.generateProjectionData('temperature', scenario, targetYear);
        ...
        const seaProjections = this.generateProjectionData('sea-level', scenario, targetYear);
    ...
    filterDataByTimeline(data) {
        return data.filter(d => d.year >= this.filters.yearStart && d.year <= this.filters.yearEnd);
    }

    generateHeatmapMatrix(data) {
        return data.map(d => ({
            year: d.year,
            anomaly: d.anomaly * (this.locationData[this.filters.location]?.tempMultiplier || 1)
        }));
    }
    ...
    startHeatmapAnimation() {
        ...
        this.animations.heatmap = setInterval(() => {
            ...
            cells.filter(d => d.year === currentYear)
                .attr('opacity', 1)
                .attr('stroke', '#ffffff')
                .attr('stroke-width', 2);
    ...
    addClimateDataToGlobe(layer) {
        ...
        const existingPoints = this.globe.scene.children.filter(child => child.userData.isClimateData);
    ...
    applyColorScheme() {
        ...
        body.className = body.className.replace(/colorblind-friendly|high-contrast/g, '');
