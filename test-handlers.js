...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (calculateBtn) {
    calculateBtn.addEventListener("click", () => {
      ...
      const milesDriven =
        parseFloat(document.getElementById("milesDriven").value) || 0;
      const gasUsage =
        parseFloat(document.getElementById("gasUsage").value) || 0;
      ...
      const totalCarbon =
        electricityCarbon + drivingCarbon + gasCarbon + flightCarbon;
  ...
  if (exportCSV) {
    exportCSV.addEventListener("click", () => {
      const csvContent =
        "Date,Temperature,SeaLevel,Emissions\n" +
        "2020,1.2,3.3,36.4\n" +
        "2021,1.3,3.4,36.8\n" +
        "2022,1.4,3.5,37.1\n" +
        "2023,1.5,3.6,37.5";
  ...
  if (exportPNG) {
    exportPNG.addEventListener("click", () => {
      ...
      const canvas = document.querySelector(
        ".visualization-section.active canvas",
      );
      if (canvas) {
        canvas.toBlob((blob) => {
          const url = URL.createObjectURL(blob);
